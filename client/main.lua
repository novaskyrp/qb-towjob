    ...
    if coords then
        QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
            QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                ...
                TriggerServerEvent('tow:server:TakeOutImpound',vehicle.plate)
                closeMenuFull()
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("tow:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "tow:client:TakeOutImpound",
                        args = {
                            vehicle = v,
        ...
        if CurrentTow == nil then
            ...
            QBCore.Functions.Progressbar("untowing_vehicle", "Remove The Vehicle", 5000, false, true, {
                ...
                if NpcOn then
                    ...
                    if #(targetPos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 25.0 then
...
local function MenuGarage()
    ...
    for k, v in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "qb-towjob:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
...
RegisterNetEvent('qb-towjob:client:SpawnVehicle', function()
...
RegisterNetEvent('qb-towjob:client:TowVehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then
            local playerped = PlayerPedId()
                ...
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", "Hoisting the Vehicle...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                QBCore.Functions.Notify("Take The Vehicle Back to The Depot", "success", 5000)
                                CurrentBlip2 = AddBlipForCoord(-140.65, -1169.51, 23.77)
                                SetBlipColour(CurrentBlip2, 3)
                                SetBlipRoute(CurrentBlip2, true)
                                SetBlipRouteColour(CurrentBlip2, 3)
                                local chance = math.random(1,100)
                                if chance < 26 then
                                    TriggerServerEvent('qb-towjob:server:nano')
                                end
                            end
                            QBCore.Functions.Notify("Vehicle Towed")
                        end, function() -- Cancel
...
RegisterNetEvent('qb-towjob:client:SelectVehicle', function()
    local coords = Config.Locations["vehicle"].coords
                    ...
                    if #(pos - vehicleCoords) < 1.5 then
                                ...
                                exports['qb-menu']:showHeader({
                                    {
                                        ...
                                        params = {
                                            event = 'qb-towjob:client:SelectVehicle',
                                            args = {}
                                        },
                                    }
                                })
                        ...
                        if IsControlJustReleased(0, 38) then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                ...
                                TriggerServerEvent('qb-towjob:server:DoBail', false)
                            end
                        end
                    end
                    sleep = 5
                ...
                if #(pos - mainCoords) < 4.5 then
                    if #(pos - mainCoords) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                RemoveBlip(CurrentBlip)
                                TriggerServerEvent("qb-towjob:server:11101110", JobsDone)
                                JobsDone = 0
                                NpcOn = false
                            else
...
RegisterNetEvent('tow:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            TriggerServerEvent("tow:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
            QBCore.Functions.DeleteVehicle(vehicle)
        end
    end
end)

RegisterNetEvent("tow:client:ImpoundMenuHeader", function (data)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('tow:client:TakeOutImpound', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['impound'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
CreateThread(function()
    ...
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "tow" then
            local pos = GetEntityCoords(PlayerPedId())
                                    ...
                                    exports['qb-menu']:showHeader({
                                        {
                                            ...
                                            params = {
                                                event = 'tow:client:ImpoundMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
